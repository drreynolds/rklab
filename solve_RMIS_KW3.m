function [tvals,Y,ns,nf] = solve_RMIS_KW3(fs,ff,tvals,Y0,hs,hf)
% usage: [tvals,Y,ns,nf] = solve_RMIS_KW3(fs,ff,tvals,Y0,hs,hf)
%
% Fixed time step RMIS-KW3, explicit+explicit multirate Runge-Kutta
% method for the vector-valued ODE problem
%     y' = fs(t,Y) + ff(t,Y), t >= t0, y in R^n,
%     Y0 = [y1(t0), y2(t0), ..., yn(t0)]'.
% The individual time steps are performed using the step_RMIS.m
% function; this routine sets the inner and outer Butcher tables to
% the KW3 table, and calls step_RMIS.m in a loop to fill the output
% arrays.
%
% Inputs:
